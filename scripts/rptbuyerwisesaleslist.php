...
if(!empty($_POST['search_date'])){ 
    ...
    $extraWhere = "`u`.`status` IN (1,2) AND `u`.orderdate = '$date' AND `uc`.`tbl_product_idtbl_product` = '$customer'";
    ...
    $extraWhere = "`u`.`status` IN (0,1) AND `uc`.`tbl_product_idtbl_product` = '$customer' AND `u`.orderdate BETWEEN '$startDate' AND '$endDate'";

}elseif(!empty($_POST['search_month'])){
    $month = $_POST['search_month'];
    $customer = $_POST['customer'];
    $month_arr = explode('-',$month);
    $extraWhere = "`u`.`status` IN (0,1) AND `uc`.`tbl_product_idtbl_product` = '$customer' AND YEAR(`u`.orderdate) = '$month_arr[0]' AND Month(`u`.orderdate) = '$month_arr[1]'";
    ...
    $extraWhere = "`u`.`status` IN (1,2) AND `uc`.`tbl_product_idtbl_product` = '$customer' AND `u`.orderdate BETWEEN '$from_date' AND '$to_date'";
}
echo json_encode(
