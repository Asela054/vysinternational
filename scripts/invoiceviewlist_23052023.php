...
if(!empty($_POST['search_date']) && !empty($_POST['search_invoice'])){ 
    ...
    $extraWhere = "`u`.`status` IN (1,2) AND `u`.invdate = '$date' AND `u`.invtype = '$invoice'";
    ...
    $extraWhere = "`u`.`status` IN (0,1) AND `u`.invtype = '$invoice' AND `u`.invdate BETWEEN '$startDate' AND '$endDate'";
    
    }elseif(!empty($_POST['search_month']) && !empty($_POST['search_invoice'])){
    $month = $_POST['search_month'];
    $month_arr = explode('-',$month);
    $invoice = $_POST['search_invoice'];
    $extraWhere = "`u`.`status` IN (0,1) AND `u`.invtype = '$invoice' AND YEAR(`u`.invdate) = '$month_arr[0]' AND Month(`u`.invdate) = '$month_arr[1]'";
    ...
    $extraWhere = "`u`.`status` IN (1,2) AND `u`.invtype = '$invoice' AND `u`.invdate BETWEEN '$from_date' AND '$to_date'";
    }
