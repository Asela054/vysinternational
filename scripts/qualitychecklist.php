...
$joinQuery = "FROM `tbl_grn` AS `u` LEFT JOIN `tbl_supplier` AS `ua` ON (`ua`.`idtbl_supplier` = `u`.`tbl_supplier_idtbl_supplier`) LEFT JOIN `tbl_location` AS `ub` ON (`ub`.`idtbl_location` = `u`.`tbl_location_idtbl_location`)";

$extraWhere = "`u`.`qualitycheck` IN (0, 1) AND `u`.`status`=1 AND `u`.`approvestatus`=1 AND `u`.`grntype`='1'";
