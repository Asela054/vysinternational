            ...
            <div class="container-fluid mt-2 p-0 p-2">
                <div class="card">
                    <div class="card-body p-0 p-2">
                        <div class="row">
                            <div class="col-3">
                                <form action="<?php echo base_url() ?>Size/Sizeinsertupdate" method="post" autocomplete="off">
                                    <div class="form-group mb-1">
                                        <label class="small font-weight-bold">Size*</label>
                                        <input type="text" class="form-control form-control-sm" name="size" id="size" required>
                                    </div>
                                    <div class="form-group mb-1">
                                        <label class="small font-weight-bold">Size Code*</label>
...
<script>
    $(document).ready(function() {
        ...
        $('#dataTable').DataTable({
            ...
            ajax: {
                url: "<?php echo base_url() ?>scripts/sizelist.php",
            ...
            "columns": [
                ...
                },
                {
                    ...
                    "render": function(data, type, full) {
                        ...
                        if(full['status']==1){
                            button+='<a href="<?php echo base_url() ?>Size/Sizestatus/'+full['idtbl_size']+'/2" onclick="return deactive_confirm()" target="_self" class="btn btn-success btn-sm mr-1 ';if(statuscheck!=1){button+='d-none';}button+='"><i class="fas fa-check"></i></a>';
                        }else{
                            button+='<a href="<?php echo base_url() ?>Size/Sizestatus/'+full['idtbl_size']+'/1" onclick="return active_confirm()" target="_self" class="btn btn-warning btn-sm mr-1 ';if(statuscheck!=1){button+='d-none';}button+='"><i class="fas fa-times"></i></a>';
                        }
                        button+='<a href="<?php echo base_url() ?>Size/Sizestatus/'+full['idtbl_size']+'/3" onclick="return delete_confirm()" target="_self" class="btn btn-danger btn-sm ';if(deletecheck!=1){button+='d-none';}button+='"><i class="fas fa-trash-alt"></i></a>';
        ...
        $('#dataTable tbody').on('click', '.btnEdit', function() {
            ...
            if (r == true) {
                ...
                $.ajax({
                    ...
                    url: '<?php echo base_url() ?>Size/Sizeedit',
                    success: function(result) { //alert(result);
                        var obj = JSON.parse(result);
                        $('#recordID').val(obj.id);
                        $('#size').val(obj.sizename);                       
                        $('#code').val(obj.sizecode);                       
