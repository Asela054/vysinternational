...
$(document).ready(function () {
		...
		"columns": [			
			...
			},
			{
				...
				"render": function (data, type, full) {
					...
					if(full['approvestatus']==0){
						button+='<a href="<?php echo base_url() ?>Semiproduction/Semiproductionstatus/'+full['idtbl_semi_production']+'/3" onclick="return delete_confirm()" target="_self" class="btn btn-danger btn-sm mr-1 ';if(statuscheck!=1){button+='d-none';}button+='"><i class="fas fa-trash-alt"></i></a>';
					}
	...
	$('#dataTable tbody').on('click', '.issuematerial', function () {
		var r = confirm("Are you sure, You want to issue material this production ? ");
        if (r == true) {
			var id = $(this).attr('id');
