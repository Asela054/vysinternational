			...
			<div class="modal-body">
				<form id="formlable">
					...
					<div class="form-group mb-1">
						...
						<select class="form-control form-control-sm" name="grnno" id="grnno" required></select>
					</div>
					<div class="form-group mb-1">
    ...
    $(document).ready(function() {
            ...
            "columns": [
                ...
                },
                {
                    ...
                    "render": function(data, type, full) {
                        var button='';
                        // button+='<button class="btn btn-warning btn-sm btnLabel mr-1" id="'+full['idtbl_material_info']+'"><i class="fas fa-tag"></i></button>';
                        button+='<button class="btn btn-secondary btn-sm btnview mr-1" id="'+full['idtbl_material_info']+'"><i class="fas fa-eye"></i></button>';
        ...
        $('#dataTable tbody').on('click', '.btnview', function(){
            ...
            $.ajax({
                ...
                url: '<?php echo base_url() ?>Materialdetail/Getmaterialinfo',
                success: function(result) { //alert(result);
        ...
        $('#dataTable tbody').on('click', '.btnLabel', function() {
            ...
            $.ajax({
                ...
                url: '<?php echo base_url() ?>Materialdetail/Getlabelinforaccomaterial',
                success: function(result) { //alert(result);
                    var obj = JSON.parse(result);
                    $('#mname').val(obj.materialname);
                    $('#mcode').val(obj.materialinfocode);

                    var objfirst = obj.grnlist;

                    var html = '';
                    html += '<option value="">Select</option>';
                    $.each(objfirst, function(i, item) {
                        //alert(objfirst[i].id);
                        html += '<option value="' + objfirst[i].idtbl_grn + '">';
                        html += 'UN/GRN-0000'+objfirst[i].idtbl_grn+' - '+objfirst[i].batchno;
                        html += '</option>';
                    });

                    $('#grnno').empty().append(html);
                }
            });
        });
        $('#grnno').change(function(){
            ...
            $.ajax({
                ...
                url: '<?php echo base_url() ?>Materialdetail/Getgrninfoaccogrnid',
                success: function(result) { //alert(result);
                    // console.log(result);
                    var obj = JSON.parse(result);
                    $('#pono').val('UN-POD-0000'+obj.tbl_porder_idtbl_porder);
                    $('#mfdate').val(obj.mfdate);
                    $('#expdate').val(obj.expdate);
        ...
        $('#btncreatelable').click(function(){
			if (!$("#formlable")[0].checkValidity()) {
				...
				var link ='<?php echo base_url() ?>Materialdetail/Createlabel/'+mname+'/'+mcode+'/'+grnno+'/'+pono+'/'+mfdate+'/'+expdate+'/'+batchno;
