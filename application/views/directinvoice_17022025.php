...
<style>
	.highlighted-row {
    background-color: yellow; /* You can choose your preferred highlight color */
}
</style>
			...
			"columns": [{
				...
				},
				{
					...
					"render": function(data, type, full) {
						...
						if(full['completestatus']==0 && statuscheck==1){
                            button+='<button type="button" data-url="Directinvoice/Completestatusupdate/'+full['idtbl_customer_porder']+'/1" data-actiontype="5" class="btn btn-success btn-sm mr-1 btntableaction"><i class="fas fa-unlock"></i></button>';
			...
			$.ajax({
				...
				success: function (result) { //alert(result);
					...
					obj.forEach(function (product) {
						options += '<option value="' + product.fgbatchno + '">' + product.fgbatchno + '/' + product.totalqty + '</option>';
					});
