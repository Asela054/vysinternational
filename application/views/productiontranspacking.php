				...
				<div class="container-fluid">
					<div class="page-header-content py-3">
						<h1 class="page-header-title font-weight-light">
							...
							<span>Production Transfer To Packing</span>
						</h1>
					</div>
				</div>
			</div>
			...
			<div class="modal-body">
                <div class="row">
                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                        <form action="<?php echo base_url() ?>Productiontranspacking/Transfertopacking" method="post">
							...
							<div class="form-row mb-2">
								<div class="col">
									<label class="small font-weight-bold text-dark">Packing Qty</label>
                                    <input type="text" id="packqty" name="packqty" class="form-control form-control-sm" value="0">
								</div>
							</div>
							<div class="form-row mb-2">
								<div class="col">
									<label class="small font-weight-bold text-dark">Semi Product</label>
                                    <select class="form-control form-control-sm" style="width: 100%;" name="semiproduct" id="semiproduct">
							...
							<div class="form-row mb-2">
								<div class="col">
									<label class="small font-weight-bold text-dark">Semi Qty</label>
                                    <input type="text" id="semiqty" name="semiqty" class="form-control form-control-sm" value="0">
								</div>
							</div>
							<div class="form-row mb-2 d-none">
								<div class="col">
									<label class="small font-weight-bold text-dark">Semi Cost Price</label>
                                    <input type="text" id="semicostprice" name="semicostprice" class="form-control form-control-sm" value="0">
								</div>
							</div>
							<div class="form-row mt-3">
								<div class="col text-right">
									<button	type="submit" id="submitBtn" class="btn btn-primary btn-sm px-4" <?php if($addcheck==0){echo 'disabled';} ?>><i class="far fa-save"></i>&nbsp;Transfer to packing</button>
								</div>
							</div>
...
<script>
	$(document).ready(function () {
		...
		$("#semiproduct").select2({
            dropdownParent: $('#modaltraspacking'),
            // placeholder: 'Select supplier',
            ajax: {
                url: "<?php echo base_url() ?>Productiontranspacking/Getmaterialinfolist",
                type: "post",
                dataType: 'json',
                delay: 250,
		...
		$('#productionorderTable').DataTable({
			...
			ajax: {
				url: "<?php echo base_url() ?>scripts/productiontranspackinglist.php",
		...
		$('#productionorderTable tbody').on('click', '.btnqualitycheck', function(){
            ...
            $.ajax({
                type: "POST",
                data: {},
                url: '<?php echo base_url() ?>Productiontranspacking/Productionpackqualityform',
		...
		$('#btnapplyquality').click(function(){
            ...
            $.ajax({
                url: '<?php echo base_url() ?>Productiontranspacking/Productionpackqualityinsertupdate',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
		...
		$('#productionorderTable tbody').on('click', '.btntranslabeling', function () {
			...
			$.ajax({
				...
				url: '<?php echo base_url() ?>Productiontranspacking/Checkissueqtyforpacking',
				success: function (result) { //alert(result);
					if(result==totalqty){
