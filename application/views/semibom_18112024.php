                ...
                <form id="bomothercost">
					<div class="form-row">
						...
						<div class="col">
							...
							<button type="button" id="btncostsubmit" class="btn btn-primary btn-sm px-3"><i class="fas fa-save mr-2"></i>Add cost</button>
							<input type="submit" class="d-none" id="hidecostsubmit">
							<input type="reset" class="d-none" id="hidecostreset">
							<input type="hidden" name="hidebomid" id="hidebomid">
							<input type="hidden" name="recordOptionCost" id="recordOptionCost" value="1">
							<input type="hidden" name="recordIDCost" id="recordIDCost" value="">
						</div>
					</div>
				</form>
				<div class="row mt-2">
					<div class="col">
						<hr>
						<table class="table table-striped table-bordered table-sm w-100" id="tablecostlist">
							<thead>
								<tr>
									<th>#</th>
									<th>Cost Type</th>
...
$(document).ready(function () {
	...
	$('#dataTable tbody').on('click', '.btnOtherCost', function () {
			...
			"columns": [{
				...
				},
				{
					...
					"render": function (data, type, full) {
						...
						if(full['status']==1 && statuscheck==1){
							button+='<button type="button" id="'+full['idtbl_semi_bom_other_cost']+'" data-actiontype="2" class="btn btn-success btn-sm mr-1 btnmodalaction"><i class="fas fa-check"></i></button>';
						}else if(full['status']==2 && statuscheck==1){
							button+='<button type="button" id="'+full['idtbl_semi_bom_other_cost']+'" data-actiontype="1" class="btn btn-warning btn-sm mr-1 text-light btnmodalaction"><i class="fas fa-times"></i></button>';
						}
						if(deletecheck==1){
							button+='<button type="button" id="'+full['idtbl_semi_bom_other_cost']+'" data-actiontype="3" class="btn btn-danger btn-sm text-light btnmodalaction"><i class="fas fa-trash-alt"></i></button>';
						}
		...
		$('#tablecostlist tbody').on('click', '.btnEdit', async function() {
			var r = await Otherconfirmation("You want to Edit this ? ");
			if (r == true) {
				var id = $(this).attr('id');
				$.ajax({
					...
					url: '<?php echo base_url() ?>Semibom/SemiBomCostEdit',
					success: function (result) { // alert(result);
						// console.log(result);
						var obj = JSON.parse(result);
						$('#recordIDCost').val(obj.id);
						$('#costamount').val(obj.perunit);
						$('#costtype').val(obj.expencestype);

						$('#recordOptionCost').val('2');
						$('#btncostsubmit').html('<i class="far fa-save"></i>&nbsp;Update');
					}
				});
			}
		});
		$('#tablecostlist tbody').on('click', '.btnmodalaction', async function() {
			var id = $(this).attr('id');
			...
			var r = await Otherconfirmation(title);
			if (r == true) {
				var id = $(this).attr('id');
				$.ajax({
					type: "POST",
					...
					url: '<?php echo base_url() ?>Semibom/SemiBomCostStatus',
					success: function (result) { // alert(result);
						var obj = JSON.parse(result);
						action(obj.action);
						if(obj.status==1){
...
function getexpencetype(id){
	$.ajax({
		...
		url: '<?php echo base_url() ?>Semibom/Getothercosttype',
		success: function(result) { //alert(result);
