...
class Invoiceviewreportinfo extends CI_Model{
    public function printreport($x){
        ...
        $sql = "SELECT `tbl_invoice_detail`.`idtbl_invoice_detail`, `tbl_invoice_detail`.`qty`, `tbl_invoice_detail`.`total`,`tbl_product`.`productcode`, `tbl_product`.`desc`, `tbl_product`.`weight`,`tbl_customer_porder_detail`.`unitprice`
    ...
    if ($respondsus->num_rows() > 0) {
        ...
        foreach($respond->result() as $rowlist){
            $tblinvoice.='
            <tr>
                    <td>'.$rowlist->idtbl_invoice_detail.'</td>
                    <td>'.$rowlist->desc.'</td>
                    <td style="text-align: center;">'.$rowlist->weight.'</td>
                    <td style="text-align: center;">'.number_format($rowlist->unitprice, 2).'</td>
    ...
    <body>
		...
		<div class="">
			<b>Date -</b> '.$invdate.' <br>
			<b>Invoice No. -</b> INV-LS/0142/2023<br>
		</div>
