...
class Directinvoiceinfo extends CI_Model{
    ...
    public function Invoiceinsertupdate(){
        ...
        $sql = "SELECT MAX(`invno`) AS `count` FROM `tbl_invoice` WHERE `invtype`= 1 AND `invno`> 0 AND `tbl_company_idtbl_company`='$companyid' AND `tbl_company_branch_idtbl_company_branch`='$branchid'";
    ...
    public function Getorderqty(){
        ...
        public function Getbatchlist(){
            ...
            $sql="SELECT `tbl_product_stock`.`idtbl_product_stock`,`tbl_product_stock`.`fgbatchno`, `qty` AS totalqty FROM `tbl_product_stock`  WHERE `tbl_product_stock`.`tbl_product_idtbl_product`=? AND `tbl_product_stock`.`status`=? AND `tbl_product_stock`.`tbl_location_idtbl_location`=? AND `tbl_product_stock`.`tbl_company_idtbl_company`=? AND `tbl_product_stock`.`tbl_company_branch_idtbl_company_branch`=? AND `qty` > ?";
            $respond=$this->db->query($sql, array($productId, 1, $fromlocation, $companyid , $branchid, 0));
