...
class Goodreceiveinfo extends CI_Model{
    ...
    public function Goodreceiveinsertupdate(){
        ...
        $sql = "SELECT COUNT(*) AS `count` FROM `tbl_grn` WHERE `grntype`= 1 AND `tbl_company_idtbl_company`='$companyid' AND `tbl_company_branch_idtbl_company_branch`='$branchid'";
    ...
    public function Getproductinfoaccoproduct(){
        ...
        if ($porderID != "") {
            ...
            $this->db->where('status', 1);
            $this->db->where('tbl_material_info_idtbl_material_info', $recordID); // corrected table name
            $respond = $this->db->get();
    
            if ($respond->num_rows() > 0) {
                $obj = new stdClass();
                $row = $respond->row(); // no need to specify index
                $obj->qty = $row->qty;
