...
class Directsaleinfo extends CI_Model{
    ...
    public function Directsaleinsertupdate(){
        ...
        $sql = "SELECT MAX(`invno`) AS `count` FROM `tbl_invoice` WHERE `invtype`= 2 AND `invno`> 0 AND `tbl_company_idtbl_company`='$companyid' AND `tbl_company_branch_idtbl_company_branch`='$branchid'";
        $respond = $this->db->query($sql);
