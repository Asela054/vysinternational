...
class Rptinvoicesummaryinfo extends CI_Model{
    ...
    public function Getinvoicedetail()
    {
        ...
        <div class="row px-2" id="print">
            ...
            foreach ($productDetails as $product) {
                ...
                <div class="card mt-2 w-100">
                    <div class="card-body p-0 p-2">
                        <h6 class="title-style mb-2 text-primary text-uppercase"><span>Invoice Details</span></h6>   
                        <br>
    
                    <div class="row">
                        <div class="col"><label class="text-dark font-weight-bold text-uppercase">Invoice No:</label> INV/DT-0000<span></span>' . $recordID . '</div>
            ...
            $html .= '
            <br>
            <h6 class="title-style mb-2 text-primary text-uppercase"><span>Materials issued to Production Order</span></h6>   
            <br>';
            ...
            if ($productDetails && $materials && $productDetails[0]->tbl_customer_porder_idtbl_customer_porder == $materials[0]->tbl_customer_porder_idtbl_customer_porder) {
                $sqlrawmaterial = "SELECT `tbl_material_info`.`idtbl_material_info`,`tbl_material_info`.`materialinfocode`, `tbl_material_code`.`materialname`, `tbl_production_material_issue`.batchno
                                    FROM `tbl_production_material_issue`
                                    LEFT JOIN `tbl_material_info` ON `tbl_material_info`.`idtbl_material_info` = `tbl_production_material_issue`.`tbl_material_info_idtbl_material_info`
                                    ...
                                    WHERE `tbl_production_material_issue`.`tbl_product_idtbl_product` = ?  AND `tbl_material_info`.`tbl_material_category_idtbl_material_category`=1 AND `tbl_production_material_issue`.`status`=1";
                ...
                if (!empty($materialinfocode)) {
                    $html .= '
                    <br>
                    <h6 class="title-style mb-2 text-primary text-uppercase"><span>Raw Material issued to Production Order</span></h6>   
                ...
                $html .= '
                <br>
                <h6 class="title-style mb-2 text-primary text-uppercase"><span>GRN Details of Materials</span></h6>              
                <br>';
    
                $sqlgrn = "SELECT `tbl_grn`.`batchno`, `tbl_grn`.`grndate`, `tbl_grndetail`.`unitprice`, `tbl_supplier`.`suppliername`
                            FROM `tbl_grn`
                            LEFT JOIN `tbl_grndetail` ON `tbl_grn`.`idtbl_grn` = `tbl_grndetail`.`tbl_grn_idtbl_grn`
                            LEFT JOIN `tbl_supplier` ON `tbl_supplier`.`idtbl_supplier` = `tbl_grn`.`tbl_supplier_idtbl_supplier`
                            WHERE `tbl_grndetail`.`tbl_material_info_idtbl_material_info` = ? AND `tbl_grn`.`batchno`=? AND `tbl_grndetail`.`status`=1";
        ...
        echo $html;
        echo '<button onclick="printInvoice()" type="button" id="formsubmit" class="btn btn-primary btn-sm ml-3 mt-3 px-4 text-right"><i class="fas fa-save"></i>&nbsp;Print</button>
        <script>
