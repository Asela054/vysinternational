...
class Materialdetailinfo extends CI_Model{
    ...
    public function Getlabelinforaccomaterial(){
        ...
        $sql="SELECT `tbl_material_code`.`materialname`, `tbl_material_info`.`materialinfocode` FROM `tbl_material_info` LEFT JOIN `tbl_material_code` ON `tbl_material_code`.`idtbl_material_code`=`tbl_material_info`.`tbl_material_code_idtbl_material_code` WHERE `tbl_material_info`.`idtbl_material_info`=?";
        $respond=$this->db->query($sql, array($recordID));

        $sqlgrnlist="SELECT `tbl_grn`.`idtbl_grn`, `tbl_grn`.`batchno` FROM `tbl_stock` LEFT JOIN `tbl_grn` ON `tbl_grn`.`batchno`=`tbl_stock`.`batchno` WHERE `tbl_stock`.`tbl_material_info_idtbl_material_info`=? AND `tbl_stock`.`qty`>?";
