...
class Directsaleinfo extends CI_Model{
    public function Getmaterial(){
        $this->db->select('`idtbl_material_code`, `materialname`');
    ...
    public function Getproductlist(){
        ...
        $sql="SELECT `tbl_product`.`idtbl_product`, `tbl_product`.`productcode`, `tbl_product`.`retailprice`, `tbl_product`.`wholesaleprice` FROM `tbl_product` LEFT JOIN tbl_material_code ON tbl_material_code.idtbl_material_code=tbl_product.materialid WHERE `tbl_product`.`materialid`= $hiddenmaterialID AND `tbl_product`.`status`=1";
        $respond=$this->db->query($sql);

        if($respond->num_rows() > 0){ 
            foreach($respond->result() as $rowlist){
                $productID=$rowlist->idtbl_product;
                $sqlstockcheck="SELECT `qty` FROM `tbl_product_stock` WHERE `tbl_product_idtbl_product`='$productID'"; 
                $respondstockcheck=$this->db->query($sqlstockcheck);
                

                if(!empty($respondstockcheck->row(0)->qty)){$stockcount=$respondstockcheck->row(0)->qty;}
                else{$stockcount=0;}  
