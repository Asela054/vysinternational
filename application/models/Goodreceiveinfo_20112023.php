...
class Goodreceiveinfo extends CI_Model{
    ...
    public function Goodreceiveview(){
        ...
        $sql="SELECT `u`.*, `ua`.`suppliername`, `ua`.`primarycontactno`, `ua`.`secondarycontactno`, `ua`.`address` AS `supplieraddress`, `ua`.`email`, `ub`.`location`, `ub`.`phone`, `ub`.`address`, `ub`.`phone2`, `ub`.`email` AS `locemail` FROM `tbl_grn` AS `u` LEFT JOIN `tbl_supplier` AS `ua` ON (`ua`.`idtbl_supplier` = `u`.`tbl_supplier_idtbl_supplier`) LEFT JOIN `tbl_location` AS `ub` ON (`ub`.`idtbl_location` = `u`.`tbl_location_idtbl_location`) WHERE `u`.`status`=? AND `u`.`idtbl_grn`=?";
        $respond=$this->db->query($sql, array(1, $recordID));

        $this->db->select('tbl_grndetail.*, tbl_material_info.materialinfocode, tbl_material_code.materialname, tbl_unit.unitcode');
        ...
        <div class="row">
            ...
            <div class="col-12 text-right">'.$respond->row(0)->suppliername.'<br>'.$respond->row(0)->primarycontactno.' / '.$respond->row(0)->secondarycontactno.'<br>'.$respond->row(0)->supplieraddress.'<br>'.$respond->row(0)->email.'</div>
            <div class="col-12">
                <hr>
                <h6>Invoice No : '.$respond->row(0)->invoicenum.'</h6>
                <h6>Dispatch No : '.$respond->row(0)->dispatchnum.'</h6>
                <h6>Batch No : '.$respond->row(0)->batchno.'</h6>
                <h6>GRN Date : '.$respond->row(0)->grndate.'</h6>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                ...
                <table class="table table-striped table-bordered table-sm">
                    ...
                    foreach($responddetail->result() as $roworderinfo){
                        ...
                        $html.='<tr>
                            ...
                            <td class="text-center">'.$roworderinfo->qty.$roworderinfo->unitcode.'</td>
    ...
    public function Getmateriallistaccogrn(){
        ...
        $sql="SELECT `tbl_material_info`.`idtbl_material_info`, `tbl_material_info`.`materialinfocode`, `tbl_material_code`.`materialname` FROM `tbl_grndetail` LEFT JOIN `tbl_material_info` ON `tbl_material_info`.`idtbl_material_info`=`tbl_grndetail`.`tbl_material_info_idtbl_material_info` LEFT JOIN `tbl_material_code` ON `tbl_material_code`.`idtbl_material_code`=`tbl_material_info`.`tbl_material_code_idtbl_material_code` WHERE `tbl_material_info`.`status`=? AND `tbl_grndetail`.`tbl_grn_idtbl_grn`=?";
        $respond=$this->db->query($sql, array(1, $recordID));
    ...
    public function Getgrninfoaccogrnid(){
        ...
        $sql="SELECT `tbl_porder_idtbl_porder`, `batchno` FROM `tbl_grn` WHERE `idtbl_grn`=?";
        $respond=$this->db->query($sql, array($recordID));
