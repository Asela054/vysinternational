...
class Customerporderinfo extends CI_Model{
    ...
    public function Customerporderinsertupdate(){
        ...
        $sql = "SELECT MAX(`sod_no`) AS `count` FROM `tbl_customer_porder` WHERE `sod_no`> 0 AND `tbl_company_idtbl_company`='$companyid' AND `tbl_company_branch_idtbl_company_branch`='$branchid'";
        $respond = $this->db->query($sql);
