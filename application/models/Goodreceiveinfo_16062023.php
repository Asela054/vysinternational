...
class Goodreceiveinfo extends CI_Model{
    ...
    public function Getgrninfoaccogrnid(){
        ...
        $sql="SELECT `tbl_grn`.`tbl_porder_idtbl_porder`, `tbl_grndetail`.`mfdate`, `tbl_grndetail`.`expdate`, `tbl_grn`.`batchno` FROM `tbl_grn` LEFT JOIN `tbl_grndetail` ON `tbl_grndetail`.`tbl_grn_idtbl_grn`=`tbl_grn`.`idtbl_grn` WHERE `tbl_grn`.`idtbl_grn`=?";
        $respond=$this->db->query($sql, array($recordID));
